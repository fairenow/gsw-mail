...
2. Set the real JWT issuer, audience and JWKS URL. Set the Stalwart bearer credential with access to all provisioned mail accounts. Set RESEND_API_KEY and the Resend webhook signing secret. Do not use a recovery username/password as a bearer token.
