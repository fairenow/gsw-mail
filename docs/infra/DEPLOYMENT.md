...
2. Set the real OIDC issuer (`OIDC_ISSUER`) and the public PKCE client id (`OIDC_CLIENT_ID=gsw-mail-web`). The API calls Stalwart's `/auth/introspect` authenticated as the trusted Stalwart mailbox service account (`STALWART_MAIL_USERNAME`/`STALWART_MAIL_PASSWORD`); that account must have access to the provisioned mail accounts. Set RESEND_API_KEY and the Resend webhook signing secret.
