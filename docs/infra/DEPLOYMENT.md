...
2. Set the real OIDC issuer (`OIDC_ISSUER`), the public PKCE client id (`OIDC_CLIENT_ID=gsw-mail-web`) and the confidential introspection client (`OIDC_INTROSPECTION_CLIENT_ID`/`OIDC_INTROSPECTION_CLIENT_SECRET`). Set the Stalwart bearer credential with access to all provisioned mail accounts. Set RESEND_API_KEY and the Resend webhook signing secret. Do not use a recovery username/password as a bearer token.
