...
| 12 | **Identity = Stalwart OIDC bearer tokens, introspected per request** | adopted | Canonical identity is the `sub` claim (`users` keyed by `identityProvider` + `identitySubject`); every request's bearer token is verified against Stalwart `/auth/introspect` (§25). Supersedes the earlier signed-JWT/JWKS plan: tokens are now issued by Stalwart's own OAuth authorization server. Dev-only fallbacks (bearer/`DEV_USER_ID`) are refused in production. |
...
| 24 | **Server-side OAuth token exchange** | adopted | The browser signs in via Stalwart OIDC PKCE (public client `gsw-mail-web`), but `POST /auth/exchange` on the API performs the `authorization_code` exchange with Stalwart, returning only the access token to the browser (`sessionStorage` for this phase). Browser -> Stalwart CORS is never needed and Stalwart CORS stays locked down. |
| 25 | **Introspection authenticates as the trusted mailbox service account** | adopted, amended | `verifyAccessToken` calls Stalwart `/auth/introspect` with Basic `username:password` from `STALWART_MAIL_USERNAME`/`STALWART_MAIL_PASSWORD` (server-only), never `Authorization: Bearer <user token>` as caller credential. Stalwart's introspection endpoint authenticates its caller as an account (`Credentials::Basic`), not as an OAuth confidential client; the originally documented `OIDC_INTROSPECTION_CLIENT_ID`/`_SECRET` client-credentials approach is not supported by Stalwart v0.16.18 and was removed. Public browser client `gsw-mail-web` stays secret-free; the introspecting caller is the server-only mailbox credential, required explicitly in production. |
