...
| 12 | **Identity = signed JWT verified via remote JWKS** | adopted | Canonical identity is the `sub` claim (`users` keyed by `identityProvider` + `identitySubject`); access checks the issuer/audience. Browser-safe: no infra secrets leave the API. Dev-only fallbacks (bearer/`DEV_USER_ID`) are refused in production. |
