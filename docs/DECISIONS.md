...
| 25 | **Introspection authenticates as a confidential client** | adopted | `verifyAccessToken` calls Stalwart `/auth/introspect` with Basic `client_id:client_secret` from `OIDC_INTROSPECTION_CLIENT_ID`/`_SECRET` (server-only), never `Authorization: Bearer <user token>` as caller credential. Public browser client `gsw-mail-web` stays secret-free; the introspecting resource-server client is confidential and refused to start without an explicit secret in production. |
