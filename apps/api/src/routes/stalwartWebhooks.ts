...
export function verifyStalwartWebhookSignature(raw: Buffer, signature: string | string[] | undefined, secret: string): boolean {
  ...
  let offered: Buffer;
  try {
    offered = Buffer.from(encoded, "base64");
  } catch {
    return false;
  }
...
async function resolveIncomingAccounts(data: Record<string, unknown>): Promise<IncomingAccount[]> {
  ...
  const principalId = typeof data.accountId === "string" || typeof data.accountId === "number"
    ? String(data.accountId)
    : null;

  const found = new Map<string, IncomingAccount>();
  for (const address of addressCandidates) {
    const rows = await db.select({ id: emailAccounts.id, address: emailAccounts.address, ownerUserId: emailAccounts.userId })
      .from(emailAccounts)
      .where(eq(emailAccounts.address, address))
      .limit(2);
  ...
  if (principalId) {
    const rows = await db.select({ id: emailAccounts.id, address: emailAccounts.address, ownerUserId: emailAccounts.userId })
      .from(emailAccounts)
      .where(or(eq(emailAccounts.stalwartPrincipalId, principalId), eq(emailAccounts.address, principalId)))
      .limit(4);
    for (const row of rows) found.set(row.id, row);
  }
...
async function notifyAccount(account: IncomingAccount, data: Record<string, unknown>) {
  const memberships = await db.select({ userId: mailAccountMemberships.userId })
    .from(mailAccountMemberships)
    .where(eq(mailAccountMemberships.accountId, account.id));
  const userIds = new Set(memberships.map((membership) => membership.userId));
  if (account.ownerUserId) userIds.add(account.ownerUserId);

  const sender = extractEmailAddresses(data.from)[0];
  const body = sender ? `New message from ${sender}` : `New message for ${account.address}`;
  const results = [];
...
export default async (app: FastifyInstance) => {
  ...
  app.post("/webhooks/stalwart", async (req, reply) => {
    ...
    try {
      payload = JSON.parse(raw.toString("utf8")) as StalwartWebhookBody;
    } catch {
