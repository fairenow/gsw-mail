...
export function MailPage() {
  ...
  const emptyTrash = async () => { if (!account || !window.confirm("Permanently delete all messages in Trash?\nThis cannot be undone.")) return; try { await api.emptyTrash(account.id); setMessages([]); setFolderCounts((current) => ({ ...current, Trash: { total: 0, unread: 0 } })); setOpen(null); void loadFolderCounts(account.id); } catch (err) { setError(err instanceof Error ? err.message : String(err)); } };
