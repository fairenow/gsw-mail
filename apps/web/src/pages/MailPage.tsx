...
export function MailPage() {
  ...
  const runAction = async (action: "archive" | "trash", engineId: string) => { if (!account) return; try { await (action === "archive" ? api.archive(account.id, engineId) : api.trash(account.id, engineId)); setOpen(null); setMobileView("messages"); await loadFolder(account.id, folder); await loadFolderCounts(account.id); } catch (err) { setError(err instanceof Error ? err.message : String(err)); } };
  const restoreMessage = async (engineId: string) => { if (!account) return; try { await api.move(account.id, engineId, "Inbox"); setOpen(null); setMobileView("messages"); await loadFolder(account.id, folder); await loadFolderCounts(account.id); } catch (err) { setError(err instanceof Error ? err.message : String(err)); } };
  const destroyMessage = async (engineId: string) => { if (!account || !window.confirm("Permanently delete this message?\nThis cannot be undone.")) return; try { await api.destroy(account.id, engineId); setOpen(null); setMobileView("messages"); await loadFolder(account.id, folder); await loadFolderCounts(account.id); } catch (err) { setError(err instanceof Error ? err.message : String(err)); } };
  const emptyTrash = async () => { if (!account || !window.confirm("Permanently delete all messages in Trash?\nThis cannot be undone.")) return; try { await api.emptyTrash(account.id); setOpen(null); await loadFolder(account.id, folder); await loadFolderCounts(account.id); } catch (err) { setError(err instanceof Error ? err.message : String(err)); } };
  ...
  const runUndo = async () => { if (!lastSend) return; try { const result = await api.cancelSend(lastSend.sendId); setLastSend({ ...lastSend, status: result.status }); } catch (err) { setError(err instanceof Error ? err.message : String(err)); } };
   ...
   return <div className="gsw-mail-shell">
    ...
    <main className={`gsw-mail-body ${sidebarCollapsed ? "sidebar-collapsed" : ""}`}>
      ...
      <section className={`gsw-message-list ${mobileView !== "messages" ? "" : "mobile-open"}`} aria-label={`${folder} messages`}><MessageListHeader folder={folder} count={messages.length} onRefresh={refresh} onEmptyTrash={folder === "Trash" ? () => void emptyTrash() : undefined} />{error && <p className="gsw-errors">{error}</p>}{messages.length ? messages.map((message) => <MessageRow key={message.engineId} message={message} folder={folder} active={open?.engineId === message.engineId} onOpen={() => void selectMessage(message)} onToggleRead={() => void toggleRead(message)} onDelete={() => void runAction("trash", message.engineId)} onRestore={folder === "Trash" ? () => void restoreMessage(message.engineId) : undefined} onDestroy={folder === "Trash" ? () => void destroyMessage(message.engineId) : undefined} />) : <div className="gsw-list-empty"><span aria-hidden="true">✉</span><strong>No messages here</strong><p>Your {folder.toLowerCase()} is clear.</p></div>}</section>
