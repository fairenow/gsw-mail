...
export function CalendarPage() {
  ...
  return <MailWorkspace section="calendar"><div className="gsw-product-shell"><main className="gsw-calendar-page">
      ...
      {dayModal && <div className="gsw-modal-backdrop gsw-calendar-day-backdrop" onMouseDown={() => setDayModal(null)}><section className="gsw-calendar-day-modal" role="dialog" aria-modal="true" aria-label={`Events for ${dayModal.toLocaleDateString()}`} onMouseDown={(event) => event.stopPropagation()}><div className="gsw-calendar-day-modal-head"><div><p className="gsw-eyebrow">Full day</p><h2>{dayModal.toLocaleDateString(undefined, { weekday: "long", month: "long", day: "numeric", year: "numeric" })}</h2></div><button className="gsw-secondary-btn" onClick={() => setDayModal(null)}>Close</button></div><div className="gsw-calendar-day-modal-actions"><button className="gsw-primary-btn" onClick={() => { openNew(dayModal); setDayModal(null); }}><Plus size={16} aria-hidden="true" /> Add event</button></div><div className="gsw-calendar-day-modal-events">{modalEvents.length ? modalEvents.map((event) => <article key={event.engineId}><div className="gsw-calendar-day-modal-time">{event.allDay ? "All day" : new Date(event.start).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" })}</div><div className="gsw-calendar-day-modal-copy"><h3>{event.title}</h3>{event.location && <p>{event.location}</p>}{event.description && <p>{event.description}</p>}{event.meetingLink && <a href={event.meetingLink} target="_blank" rel="noreferrer">Open virtual meeting</a>}</div><div className="gsw-calendar-day-modal-event-actions"><button className="gsw-secondary-btn" onClick={() => { openEdit(event); setDayModal(null); }}>Edit</button><button className="gsw-link-btn" onClick={() => void remove(event)}>Delete</button></div></article>) : <div className="gsw-calendar-day-empty"><strong>No events</strong><p>This day is open.</p></div>}</div></section></div>}
...
function CalendarGrid({ events, month, selectedDay, view, onSelectDay, onOpen, onDelete }: { events: CalendarEvent[]; month: Date; selectedDay: Date; view: CalendarView; onSelectDay: (day: Date) => void; onOpen: (event: CalendarEvent) => void; onDelete: (event: CalendarEvent) => void }) {
  ...
  return <section className={`gsw-calendar-grid gsw-calendar-grid-${view}`}>{dates.map((day) => {
    ...
    return <div className={`gsw-calendar-day${outsideMonth ? " gsw-calendar-day-outside-month" : ""}`} key={key} onClick={() => onSelectDay(day)}><header><strong>{day.toLocaleDateString(undefined, { weekday: view === "month" ? "short" : "long", month: "short", day: "numeric" })}</strong></header>{view === "month" ? <div className="gsw-calendar-month-events">{monthEvents.map((event) => <div className="gsw-calendar-month-event" key={event.engineId}><time>{event.allDay ? "All day" : new Date(event.start).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" })}</time><span>{event.title}</span></div>)}{dayEvents.length > 2 && <div className="gsw-calendar-more-events">+{dayEvents.length - 2} more</div>}</div> : dayEvents.map((event) => <div className="gsw-calendar-grid-event" key={event.engineId}><button onClick={(click) => { click.stopPropagation(); onOpen(event); }}><time>{event.allDay ? "All day" : new Date(event.start).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" })}</time><span>{event.title}</span>{event.meetingLink && <small>Virtual</small>}</button><button className="gsw-calendar-grid-delete" onClick={(click) => { click.stopPropagation(); void onDelete(event); }}>Delete</button></div>)}</div>;
  })}</section>;
}
