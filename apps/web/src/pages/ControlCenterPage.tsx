...
export function ControlCenterPage() {
  ...
  return <main className="gsw-control-center">
    ...
    <section className="gsw-control-main"><header><div><p className="gsw-setup-kicker">Control center</p><h2>{workspace?.name ?? "Guided Steps Wellness"}</h2><p>{workspace?.role ?? "member"} · {context.user.email}</p></div><button className="gsw-btn gsw-btn-primary" onClick={() => window.location.assign("/mail")}>Open Mail</button></header><div className="gsw-control-stats"><div><strong>{domains.size}</strong><span>Domains</span></div><div><strong>{context.mailboxMemberships.length}</strong><span>Mailboxes</span></div><div><strong>{context.workspaceMemberships.length}</strong><span>Workspaces</span></div></div><div className="gsw-control-grid"><article><h3>Domains</h3><p>{domains.size ? [...domains].join(" · ") : "No domains connected"}</p><span className="gsw-status-pill">{domains.size ? "Connected" : "Needs setup"}</span></article><article><h3>Mailboxes</h3><p>{context.mailboxMemberships.length ? `${context.mailboxMemberships.length} mailbox membership${context.mailboxMemberships.length === 1 ? "" : "s"}` : "Control identity has no mailbox access"}</p><span className="gsw-status-pill">{context.mailboxMemberships.length ? "Active" : "Admin only"}</span></article><article><h3>Setup</h3><p>{context.onboardingComplete ? "Existing installation adopted" : "Workspace setup needs attention"}</p><span className="gsw-status-pill">{context.onboardingComplete ? "Complete" : "In progress"}</span></article><article><h3>Mailbox access</h3><p>{context.mailboxMemberships.length ? "Explicit memberships only" : "No mailbox content access"}</p><span className="gsw-status-pill">Secure</span></article></div></section>
  </main>;
}
