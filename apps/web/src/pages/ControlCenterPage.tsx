...
type AdminSummary = {
  ...
  users: {
    ...
    mailboxAccess: { accountId: string; address: string; role: string }[];
  }[];
  mailboxes: {
    id: string;
    address: string;
...
function statusTone(value: string) {
  return value === "verified" || value === "ready" || value === "active" ? "good" : value === "failed" || value === "disabled" || value === "suspended" ? "bad" : "warn";
}
...
export function ControlCenterPage() {
  ...
  useEffect(() => {
    void getAccountContext().then(setContext).catch(() => undefined);
  }, []);
  ...
  useEffect(() => {
    ...
    fetch(`/admin/control-center?organizationId=${encodeURIComponent(workspace.id)}`, { credentials: "include", signal: AbortSignal.timeout(15000) })
      .then(async (response) => {
        ...
        if (!response.ok) throw new Error(result.error ?? "Could not load workspace administration.");
        if (active) setAdmin(result as AdminSummary);
      })
  ...
  const navigate = (next: ControlSection) => {
    setSection(next);
  ...
  const reloadAdmin = async () => {
    ...
    const result = await response.json();
    if (!response.ok) throw new Error(result.error ?? "Could not refresh workspace administration.");
    setAdmin(result as AdminSummary);
  };
    ...
    <section className="gsw-control-main">
      ...
      {section === "overview" && <>
        <div className="gsw-control-stats">
          <button onClick={() => navigate("domains")}><strong>{managedDomains.length}</strong><span>Domains</span><small>{admin?.domains.some((domain) => domain.status !== "verified") ? "Review DNS status" : "Connected"}</small></button>
        ...
        <div className="gsw-control-grid">
          <article className="gsw-control-action-card">
            <p className="gsw-setup-kicker">Mailbox readiness</p>
            <h3>{pendingMailboxes ? `${pendingMailboxes} mailbox${pendingMailboxes === 1 ? "" : "es"} need attention` : "All mailbox logins are ready"}</h3>
            <p>{pendingMailboxes ? "Finish login setup so each mailbox can sign in independently." : "Passwords can be reset securely from the Mailboxes section using admin OTP verification."}</p>
            <button className="gsw-btn gsw-btn-quiet" onClick={() => navigate("mailboxes")}>{pendingMailboxes ? "Finish mailbox setup" : "Manage mailboxes"}</button>
          </article>
          <article className="gsw-control-action-card">
            <p className="gsw-setup-kicker">Domain health</p>
            <h3>{admin?.domains.length ? admin.domains.map((domain) => domain.name).join(" · ") : managedDomains.join(" · ") || "No domain connected"}</h3>
            <p>{admin?.domains.some((domain) => domain.status !== "verified") ? "One or more domain checks need attention." : "Review the DNS records GSW currently has recorded for your domain."}</p>
            <button className="gsw-btn gsw-btn-quiet" onClick={() => navigate("domains")}>Review domains</button>
          </article>
          <article className="gsw-control-action-card">
            <p className="gsw-setup-kicker">Account recovery</p>
            <h3>{admin?.recoveryAdmin?.email ?? adminEmail}</h3>
            <p>This verified workspace owner identity is used to recover mailbox credentials when mailbox access itself is unavailable.</p>
            <button className="gsw-btn gsw-btn-quiet" onClick={() => navigate("security")}>Review security</button>
          </article>
          <article className="gsw-control-action-card">
            <p className="gsw-setup-kicker">Workspace access</p>
            <h3>{admin?.users.filter((user) => user.membershipStatus === "active").length ?? 1} active user{(admin?.users.filter((user) => user.membershipStatus === "active").length ?? 1) === 1 ? "" : "s"}</h3>
            <p>See who belongs to the workspace, their role, and every mailbox they can access.</p>
            <button className="gsw-btn gsw-btn-quiet" onClick={() => navigate("users")}>Review users</button>
          </article>
        </div>
      </>}

      {section === "domains" && <section className="gsw-control-section">
        <div className="gsw-control-section-head"><div><p className="gsw-setup-kicker">Domains</p><h3>Domain & DNS health</h3><p>These are the delivery and authentication records currently tracked for your workspace.</p></div></div>
        {admin?.domains.length ? admin.domains.map((domain) => <article className="gsw-domain-card" key={domain.id}>
          <div className="gsw-control-row-head"><div><h4>{domain.name}</h4><p>Last recorded update {formatDate(domain.updatedAt)}</p></div><span className={`gsw-status-pill tone-${statusTone(domain.status)}`}>{humanStatus(domain.status)}</span></div>
          <div className="gsw-dns-grid">
            {[["MX", domain.mxStatus], ["SPF", domain.spfStatus], ["DKIM", domain.dkimStatus], ["DMARC", domain.dmarcStatus]].map(([label, value]) => <div key={label}><strong>{label}</strong><span className={`gsw-status-dot tone-${statusTone(value)}`}>{humanStatus(value)}</span></div>)}
          </div>
          {domain.dkimSelector ? <p className="gsw-control-note">DKIM selector: <code>{domain.dkimSelector}</code></p> : null}
        </article>) : <div className="gsw-control-empty"><strong>No connected domain details yet.</strong><p>Domain setup can be completed from the workspace setup flow.</p></div>}
      </section>}
      ...
      {section === "mailboxes" && <section className="gsw-control-section">
        ...
        <div className="gsw-admin-list">
          {context.managedMailboxes.length ? context.managedMailboxes.map((mailbox) => {
            const details = admin?.mailboxes.find((item) => item.id === mailbox.id);
            return <article className="gsw-admin-list-row" key={mailbox.id}>
              <div className="gsw-admin-primary"><strong>{mailbox.address}</strong><span>{mailbox.displayName ?? "Mailbox"}</span></div>
              <div className="gsw-admin-meta"><span className={`gsw-status-pill tone-${statusTone(mailbox.authSetupStatus)}`}>{mailbox.authSetupStatus === "ready" ? "Login ready" : "Setup required"}</span><small>{details?.accessCount ?? 0} user{(details?.accessCount ?? 0) === 1 ? "" : "s"} with access</small>{details ? <small>{formatBytes(details.usedBytes)} used{details.quotaBytes ? ` of ${formatBytes(details.quotaBytes)}` : ""}</small> : null}</div>
              <button className="gsw-btn gsw-btn-quiet" disabled={setupBusy} onClick={() => void startMailboxSetup(mailbox)}>{mailbox.authSetupStatus === "ready" ? "Reset login" : "Set up login"}</button>
            </article>;
      ...
      {section === "users" && <section className="gsw-control-section">
        ...
        {admin ? <div className="gsw-user-grid">{admin.users.map((user) => <article className="gsw-user-card" key={user.membershipId}>
          <div className="gsw-control-row-head"><div><h4>{user.name || user.email}</h4><p>{user.authEmail || user.email}</p></div><span className={`gsw-role-pill role-${user.role}`}>{humanStatus(user.role)}</span></div>
          <div className="gsw-user-facts"><span><strong>Status</strong>{humanStatus(user.membershipStatus)}</span><span><strong>Verified</strong>{user.authEmailVerified || user.emailVerified ? "Yes" : "No"}</span><span><strong>Last login</strong>{formatDate(user.lastLoginAt)}</span></div>
          <div className="gsw-access-block"><strong>Mailbox access</strong>{user.mailboxAccess.length ? user.mailboxAccess.map((access) => <div key={`${user.userId}-${access.accountId}`}><span>{access.address}</span><small>{humanStatus(access.role)}</small></div>) : <p>No mailbox access assigned.</p>}</div>
        </article>)}</div> : <div className="gsw-control-empty">Loading workspace users…</div>}
      </section>}
      ...
      {section === "security" && <section className="gsw-control-section">
        ...
        <div className="gsw-security-grid">
          <article className="gsw-security-card"><span className="gsw-security-icon">✓</span><div><strong>Workspace recovery email</strong><h4>{admin?.recoveryAdmin?.email ?? adminEmail}</h4><p>{admin?.recoveryAdmin?.emailVerified === false ? "Verification required" : "Verified workspace owner identity"}</p></div></article>
          <article className="gsw-security-card"><span className="gsw-security-icon">#</span><div><strong>Mailbox credential recovery</strong><h4>6-digit OTP</h4><p>Codes expire after 10 minutes and are limited to five attempts.</p></div></article>
        </div>
      ...
      {setupMailbox && <div className="gsw-modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !setupBusy) setSetupMailbox(null); }}>
        <div className="gsw-control-modal" role="dialog" aria-modal="true" aria-labelledby="mailbox-setup-title">
          <p className="gsw-setup-kicker">Admin verified mailbox access</p>
          <h3 id="mailbox-setup-title">{setupAction === "reset" ? "Reset" : "Set up"} {setupMailbox.address}</h3>
          ...
          <div className="gsw-control-modal-actions">
