...
export function MessageRow({ message, active, onOpen }: { message: MessageSummary; active: boolean; onOpen: () => void }) {
  ...
  return (
    <article className={`gsw-message-row ${message.read ? "read" : "unread"} ${active ? "open" : ""}`} onClick={onOpen} tabIndex={0} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") onOpen(); }}>
      ...
      <div className="gsw-row-actions"><button onClick={(event) => event.stopPropagation()} aria-label="Star message">☆</button><button onClick={(event) => event.stopPropagation()} aria-label="More message actions">⋯</button></div>
    </article>
  );
}
