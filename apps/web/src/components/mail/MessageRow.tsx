...
export function MessageRow({ message, active, selected, folder, onOpen, onPrefetch, onSelect, onToggleRead, onToggleFlag, onDelete, onArchive, onRestore, onDestroy }: {
  ...
  return (
    <article className={`gsw-message-row ${message.read ? "read" : "unread"} ${active ? "open" : ""} ${selected ? "selected" : ""}`} onClick={onOpen} onPointerEnter={prefetch} onFocus={prefetch} tabIndex={0} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") onOpen(); }}>
      <input className="gsw-message-select" type="checkbox" checked={selected} onChange={onSelect} onClick={(event) => event.stopPropagation()} aria-label={`Select ${message.subject || "message"}`} />
      ...
      <button className="gsw-message-star" type="button" onClick={(event) => { event.stopPropagation(); onToggleFlag(); }} aria-label={message.flagged ? "Unstar message" : "Star message"} title={message.flagged ? "Unstar" : "Star"}><Star size={17} fill={message.flagged ? "currentColor" : "none"} /></button>
